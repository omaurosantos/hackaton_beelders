...
interface DashboardSummary {
  ...
  taxa_risco: number; courses: CourseOption[];
}
interface Student {
  id: number; name: string; course: string; risk_score: number; risk_level: string;
  debtor: number; tuition_fees_up_to_date: number;
  curricular_units_1st_sem_grade: number; curricular_units_2nd_sem_grade: number;
}
    ...
    <div className="min-h-screen bg-fog-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-5 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 sm:mb-8">
          <div>
            <h2 className="font-bold text-fog-900 text-xl sm:text-2xl">Dashboard do Professor</h2>
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="flex-1 sm:flex-none text-sm rounded-lg px-3 py-2 bg-white font-medium text-fog-700 min-w-0"
            ...
            <label className="z-btn z-btn--sm flex items-center gap-1.5 cursor-pointer shrink-0"
              style={{ background: "white", color: "var(--fg-muted)", border: "1.5px solid var(--fog-200)" }}>
              <UploadSimple size={15} weight="bold" />
              <span className="hidden sm:inline">{uploading ? "Enviando..." : "Importar CSV"}</span>
        ...
        {/* Risk bar */}
        <div className="z-card mb-5 sm:mb-6 p-4 sm:p-6">
          <p className="text-sm font-semibold text-fog-700 mb-3">Distribuição de risco na turma</p>
          <div className="flex rounded-full overflow-hidden h-3">
            <div style={{ width: `${(summary.alto_risco / summary.total) * 100}%`, background: "var(--attention-danger)", transition: "width 300ms ease-out" }} />
            <div style={{ width: `${(summary.medio_risco / summary.total) * 100}%`, background: "var(--attention-warning)", transition: "width 300ms ease-out" }} />
            <div style={{ width: `${(summary.baixo_risco / summary.total) * 100}%`, background: "var(--attention-success)", transition: "width 300ms ease-out" }} />
          </div>
          <div className="flex gap-4 mt-2.5 text-xs text-fog-500">
            {[{ label: "Alto", color: "var(--attention-danger)" }, { label: "Médio", color: "var(--attention-warning)" }, { label: "Baixo", color: "var(--attention-success)" }]
              .map(({ label, color }) => (
        ...
        <div className="bg-white rounded-lg overflow-hidden" style={{ boxShadow: "var(--shadow-sm)" }}>
          ...
          <div className="px-4 sm:px-5 py-3 sm:py-4" style={{ borderBottom: "1px solid var(--fog-100)" }}>
            ...
            {showFilters && (
              <div className="flex flex-wrap gap-2 mt-3 sm:hidden">
                {FILTERS.map(({ key, label, count }) => (
                  <button key={key} onClick={() => { setRiskFilter(key); setShowFilters(false); }}
                    className="z-btn z-btn--sm"
                    style={riskFilter === key
                      ? { background: "var(--primary)", color: "white" }
                      : { background: "var(--fog-100)", color: "var(--fg-muted)" }}>
          ...
          {/* Mobile: card list */}
          <div className="sm:hidden divide-y" style={{ borderColor: "var(--fog-100)" }}>
            {paginated.map((s) => (
              <div key={s.id} className="p-4 flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-fog-900 text-sm truncate">{s.name}</p>
                  <p className="text-fog-400 text-xs mt-0.5 truncate">{s.course}</p>
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-fog-500 text-xs">nota {gradeAvg(s)}</span>
                  </div>
                </div>
                <button onClick={() => router.push(`/student/${s.id}`)}
                  className="z-btn z-btn--sm z-btn--secondary shrink-0">
                  Ver
                </button>
              </div>
            ))}
            {paginated.length === 0 && (
              <p className="px-4 py-8 text-center text-fog-400 text-sm">Nenhum aluno neste filtro.</p>
            )}
          </div>
              ...
              <tbody>
                {paginated.map((s) => (
                  <tr key={s.id} className="border-t"
                    ...
                    <td className="px-5 py-3.5 text-center"><RiskBadge level={s.risk_level} /></td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 sm:px-5 py-3"
              ...
              <span className="text-xs text-fog-500">
                {page}/{totalPages} · {filtered.length} alunos
              </span>
              <div className="flex items-center gap-1.5">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                  className="w-8 h-8 rounded-md flex items-center justify-center text-fog-600 disabled:opacity-40"
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((n, i) => n === "…"
                    ? <span key={`e-${i}`} className="text-xs text-fog-400 px-0.5">…</span>
